...
const BoostersPage: React.FC = () => {
  ...
  const getBoostersQueryByName = async () => {
    try {
      const response = await fetch(`http://localhost:5001/boosters/querybyname/${searchBarInput}`);
      const jsonData = await response.json();
      setBoosters(jsonData);
    } catch (error) {
      console.error(error.message);
  ...
  return (
    <Container>
      ...
      <SearchBar onSubmit={getBoostersQueryByName} onChange={handleChange} inputValue={searchBarInput}/>
    </Container>
  );
};
